...
class BaseHandler(object):
    ...
    def _crawl(self, url, **kwargs):
        ...
        if kwargs.get("data_fetch_content"):
            task['data_fetch_content'] = kwargs["data_fetch_content"]

        if kwargs.get("pass_content"):
            task['pass_content'] = kwargs["pass_content"]
    ...
    def on_result(self, result):
        ...
        if self.__env__.get('result_queue'):
            if isinstance(result, list):
                for i in result:
                    self.__env__['result_queue'].put((self.task, i))
            else:
