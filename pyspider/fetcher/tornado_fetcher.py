...
class Fetcher(object):
    ...
    def __init__(self, inqueue, outqueue, poolsize=100, proxy_file=None, async=True):
    ...
    def data_fetch(self, url, task, callback):
        ...
        result['data_fetch_content'] = task.get("data_fetch_content", {})
        result['pass_content'] = task.get("pass_content", {})
    ...
    def http_fetch(self, url, task, callback):
        ...
        proxy_string = None
        if self.proxy_file and task_fetch.get('proxy', True):
            proxy_list = []
            # TODO: low efficiency
            # It reads entire proxy_file every fetch
            # Perhaps there should be a Proxy class which manager the proxy
            with open(self.proxy_file, 'r') as fh:
                proxy_list = fh.readlines()
            proxy_string = (random.choice(proxy_list)).strip('\n')
        if proxy_string:
